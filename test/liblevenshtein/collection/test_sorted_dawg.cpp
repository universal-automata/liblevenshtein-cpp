...
auto is_sorted(std::vector<std::string> &terms) -> bool {
...
RC_GTEST_PROP(SortedDawg, rejects_unsorted_terms,
              (const std::set<std::string> &terms, unsigned seed)) {
  RC_PRE(terms.size() > 1);
...
RC_GTEST_PROP(SortedDawg, contains_expected_terms,
              (std::set<std::string, std_str_cmp> terms_to_add,
               const std::set<std::string, std_str_cmp> &terms_to_ignore)) {
